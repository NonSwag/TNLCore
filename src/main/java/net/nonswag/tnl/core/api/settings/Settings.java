package net.nonswag.tnl.core.api.settings;

import net.nonswag.tnl.core.api.config.PropertyConfig;
...
public abstract class Settings {
    ...
    @Nonnull
    private static final PropertyConfig config = new PropertyConfig("Listener", "settings.properties");
    ...
    public static void init() {
        for (Setting<?> setting : Setting.getList()) {
            if (getConfig().has(setting.getKey())) {
                ...
                } else if (setting.getValue() instanceof Boolean value) {
                    getConfig().setValue(setting.getKey(), value);
                } else if (setting.getValue() instanceof Byte value) {
                    getConfig().setValue(setting.getKey(), value);
                } else if (setting.getValue() instanceof List) {
                    List<String> value = (List<String>) setting.getValue();
                    getConfig().setValue(setting.getKey(), value);
                } else if (setting.getValue() instanceof Integer value) {
                    getConfig().setValue(setting.getKey(), value);
                } else {
